...
r = run("git tag --points-at HEAD", capture_output=True, encoding="utf8", check=True, shell=True)
is_tagged = r.stdout.strip()
if is_tagged:
    version = __version__
else:
    r = run("git rev-parse HEAD", capture_output=True, encoding="utf8", check=True, shell=True)
